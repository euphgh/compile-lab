...
const var_t VarDec_c(node_t& node, const type_t* inh_type) {
    if (node.synt_sym == ID) {
        ...
        const var_t sub_var = VarDec_c(get_node(node.cld_idx[0]), inh_type);
        unsigned array_num = get_node(node.cld_idx[2]).attrib.cnt_int;
        return var_t {
            .name = sub_var.name,
...
var_t ParamDec_c(node_t& node) {
    return VarDec_c(get_node(node.cld_idx[1]), 
            Specifier_c(get_node(node.cld_idx[0])));
}
...
var_table VarList_c(node_t& node) {
    var_table res {};
    if (node.cld_nr==1)
        res.insert(ParamDec_c(get_node(node.cld_idx[0])));
    else {
        res.insert(ParamDec_c(get_node(node.cld_idx[0])));
        bool deplicate = res.append(VarList_c(get_node(node.cld_idx[2])));
...
void FunDec_c(node_t& node, const type_t* inh_ret_type) {
    string func_name = get_node(node.cld_idx[0]).attrib.id_lit;
    ...
    if (node.cld_nr == 3) { // no para
        ...
        var_table para_table = VarList_c(get_node(node.cld_idx[2]));
        g_func_tbl.insert(func_t{
            .name = func_name,
            .ret_type = inh_ret_type,
            .para = para_table,
